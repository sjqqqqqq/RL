...
const D_F   = 32                # feature dimension (dangling leg) — paper Study A
const CENTRAL_SITE = N ÷ 2      # site that carries the feature leg

# Paper Study A (Tab. II + Fig. 2 caption): 12 actions = 6 generators × 2 signs.
# Asymmetric step sizes (δt₊ ≠ |δt₋|) prevent the agent from trivially undoing
# an action by applying its inverse (supplementary p. 20).
const GENERATORS = ["X", "Y", "Z", "XX", "YY", "ZZ"]
const SIGNS      = [+1.0, -1.0]
const DT_PLUS    = π/8           # δt₊ for positive-sign actions
const DT_MINUS   = π/16          # δt₋ for negative-sign actions (|δt₋| ≠ δt₊)
const N_ACTIONS  = length(GENERATORS) * length(SIGNS)

# Index layout: a-1 = (gen_idx-1)*nSign + (sign_idx-1)
function decode_action(a::Int)
    a0 = a - 1
    nSign = length(SIGNS)
    sign_idx = a0 % nSign + 1
    gen_idx  = a0 ÷ nSign + 1
    dt = SIGNS[sign_idx] > 0 ? DT_PLUS : DT_MINUS
    return GENERATORS[gen_idx], SIGNS[sign_idx], dt
end
...
struct RegEntry
    ψ::MPS
    dense::Array{ComplexF64,4}            # (s1,s2,s3,s4) at canonical site order
end
...
function _qmps_feature(ψc::AbstractArray{ComplexF64,4},
    ...
    res = [log(abs2(c) + 1e-16) / N for c in o]
    return res
end
...
function qmps_feature_and_vjp_batch(state_ids::AbstractVector)
    # Dense conjugated states pulled from the registry cache.
