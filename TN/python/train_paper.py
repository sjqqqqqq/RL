...
# bridge_paper imports juliacall, which must come before torch to avoid the
# "torch was imported before juliacall" segfault risk (and the module-replace
# warning that was masking the D_F=8 vs D_F=32 collision earlier).
import bridge_paper as B
from qmps_agent_paper import QMPSDQN, clone_for_target, hard_copy
...
def parse_args() -> argparse.Namespace:
    # Defaults match Metz & Bukov 2023 Tab. I/II (Study A, QMPS-1).
    ...
    p.add_argument("--out-prefix",     type=str,   default="study_a_paper")
...
def train(args: argparse.Namespace) -> None:
    ...
    for ep in range(args.episodes):
        # Paper Tab. I: eps_l = eps_min + (eps_init - eps_min) * exp(-8l/N_eps)
        eps = args.eps_min + (args.eps_init - args.eps_min) * math.exp(
            -8.0 * ep / max(1, args.episodes)
        )
